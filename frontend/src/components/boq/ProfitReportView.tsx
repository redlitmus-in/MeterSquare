  ...
  const handleDownloadLabourPDF = async () => {
    ...
    try {
      ...
      if (plannedWorkers.length > 0) {
        ...
        autoTable(doc, {
          startY: yPos,
          head: [['Role', 'BOQ Item', 'Planned Hours', 'Rate / Hr', 'Planned Cost']],
          ...
          foot: [['Total Planned Labour', '', '', '', `AED ${planned.toLocaleString('en-AE', { minimumFractionDigits: 2 })}`]],
          theme: 'striped',
          headStyles: { fillColor: [59, 130, 246], textColor: 255, fontSize: 7, fontStyle: 'bold' },
          bodyStyles: { fontSize: 7 },
      ...
      if (workers.length > 0) {
        ...
        autoTable(doc, {
          startY: yPos,
          head: [['Code', 'Worker Name', 'Role', 'Phone', 'Work Days', 'Reg. Hrs', 'OT Hrs', 'Total Hrs', 'Rate/Hr', 'Total Cost']],
          ...
          foot: [[
            ...
            String(summary.total_working_days || 0),
            (summary.regular_hours || 0).toFixed(1),
            (summary.overtime_hours || 0).toFixed(1),
            (summary.total_hours || 0).toFixed(1), '',
            `AED ${actual.toLocaleString('en-AE', { minimumFractionDigits: 2 })}`
          ]],
          theme: 'striped',
      ...
      if (requisitions.length > 0) {
        ...
        autoTable(doc, {
          startY: yPos,
          head: [['Req Code', 'Site', 'Date', 'Skill / Role', 'Requested By', 'Req.', 'Assigned', 'Attended', 'Hours', 'Cost', 'Status']],
          ...
          bodyStyles: { fontSize: 6.5 },
          columnStyles: { 5: { halign: 'center' }, 6: { halign: 'center' }, 7: { halign: 'center' }, 8: { halign: 'right' }, 9: { halign: 'right' }, 10: { halign: 'center' } },
          margin: { left: margin, right: margin }
        });
      }
                          ...
                          <table className="w-full text-sm">
                            <thead className="bg-gray-50 border-b border-gray-200">
                              <tr>
                                ...
                                <th className="text-right px-4 py-3 text-xs font-semibold text-gray-500 uppercase">Plan Qty</th>
                                <th className="text-right px-4 py-3 text-xs font-semibold text-gray-500 uppercase">Plan Rate</th>
                                <th className="text-right px-4 py-3 text-xs font-semibold text-gray-500 uppercase">Plan Amount</th>
                                ...
                                <th className="text-center px-4 py-3 text-xs font-semibold text-gray-500 uppercase">Variance</th>
                              </tr>
                            </thead>
                            <tbody className="divide-y divide-gray-100">
                              {data.materials.details.map((row: any, idx: number) => {
                                ...
                                <tr key={idx} className={`transition-colors ${isCR ? 'bg-orange-50 hover:bg-orange-100' : 'hover:bg-gray-50'}`}>
                                  ...
                                  <td className="px-4 py-3 text-right text-gray-400 italic">{isCR ? 'New' : (row.planned_quantity > 0 ? row.planned_quantity.toFixed(2) : '-')}</td>
                                  <td className="px-4 py-3 text-right text-gray-400 italic">{isCR ? '-' : (row.planned_rate > 0 ? formatCurrency(row.planned_rate) : '-')}</td>
                                  <td className="px-4 py-3 text-right font-semibold text-gray-400 italic">{isCR ? '-' : formatCurrency(row.planned_amount)}</td>
                                  ...
                                  <td className="px-4 py-3 text-center">
                                    {isPending ? (
                                      ...
                                      <VarianceBadge variance={row.variance} status={row.status} />
                                    )}
                                  </td>
                                </tr>
                                );
                            ...
                            <tfoot className="bg-gray-50 border-t-2 border-gray-200">
                              <tr>
                                ...
                                <td className="px-4 py-3 text-center">
                                  <VarianceBadge
                                    variance={data.materials?.variance || 0}
                                    status={data.materials?.actual > data.materials?.planned ? 'over' : 'under'}
                                  />
                                </td>
                              </tr>
                            </tfoot>
