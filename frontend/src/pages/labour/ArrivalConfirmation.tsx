...
const ArrivalConfirmation: React.FC = () => {
  ...
  // Convert grouped arrivals to array for pagination
  const groupedArrivalsArray = Object.entries(groupedArrivals);
  const totalRequisitions = groupedArrivalsArray.length;
  const totalPages = Math.ceil(totalRequisitions / PAGINATION.DEFAULT_PAGE_SIZE);

  // Paginated requisition groups
  const paginatedGroups = groupedArrivalsArray.slice(
    (currentPage - 1) * PAGINATION.DEFAULT_PAGE_SIZE,
    currentPage * PAGINATION.DEFAULT_PAGE_SIZE
  );

  // Reset page when arrivals change
  React.useEffect(() => {
    setCurrentPage(1);
  }, [arrivals.length, selectedProject?.project_id, selectedDate]);
    ...
    <div className="p-6 max-w-7xl mx-auto">
      ...
      {!selectedProject ? (
        ...
        <div className="space-y-6">
          {paginatedGroups.map(([reqId, group]) => {
          ...
          {totalRequisitions > 0 && (
            <div className="bg-white px-4 py-3 flex items-center justify-between border border-gray-200 rounded-lg shadow-sm mt-4">
              <div className="text-sm text-gray-700">
                ...
                {Math.min(currentPage * PAGINATION.DEFAULT_PAGE_SIZE, totalRequisitions)} of{' '}
