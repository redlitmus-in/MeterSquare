...
const AssetDisposalPage: React.FC = () => {
  ...
  const fetchDisposalItems = async () => {
    setLoading(true);
    try {
      // Fetch both pending and approved disposal requests
      const disposals = await getAssetDisposalRequests('all');
      const filteredDisposals = (disposals || []).filter(
        (item: AssetDisposalRequest) => item.status === 'pending_review' || item.status === 'approved'
      );
      setDisposalItems(filteredDisposals);
    } catch (error) {
  ...
  const pendingCount = disposalItems.filter(item => item.status === 'pending_review').length;
  const approvedCount = disposalItems.filter(item => item.status === 'approved').length;
            ...
            <div className="p-12 text-center">
              ...
              <p className="text-sm text-gray-500">
                {searchTerm
                  ...
                  : 'No TD-approved assets awaiting disposal'}
              </p>
            </div>
          )}
        </div>
