...
const ChangeRequestsPage: React.FC = () => {
  ...
  const loadChangeRequests = async () => {
    setLoading(true);
    try {
      const response = await changeRequestService.getChangeRequests('estimator');
      if (response.success) {
  ...
  const handleApprove = async (boqId: number) => {
    try {
      const response = await changeRequestService.approveByEstimator(boqId);
      if (response.success) {
  ...
  const filteredRequests = changeRequests.filter(req => {
    ...
    const matchesTab = (
      (activeTab === 'pending' && req.status === 'pending' && req.approval_type === 'estimator') ||
      (activeTab === 'approved' && (req.status === 'approved_estimator' || req.status === 'approved_td')) ||
      (activeTab === 'escalated' && req.approval_type === 'td') ||
  ...
  const stats = {
    pending: changeRequests.filter(r => r.status === 'pending' && r.approval_type === 'estimator').length,
    approved: changeRequests.filter(r => r.status === 'approved_estimator' || r.status === 'approved_td').length,
    escalated: changeRequests.filter(r => r.approval_type === 'td' && r.status === 'pending').length,
            ...
            <TableRow key={request.cr_id}>
              ...
              <TableCell className="text-right">
                <div className="flex items-center justify-end gap-2">
                  ...
                  {request.status === 'pending' && request.approval_type === 'estimator' && (
                    <>
                    ...
                    {filteredRequests.map((request, index) => (
                      ...
                      >
                        ...
                        <div className="px-4 pb-3 text-center text-sm">
                          <span className="font-bold text-blue-600 text-lg">{request.new_items_count}</span>
                          ...
                          <div className="bg-gradient-to-r from-purple-50 to-purple-100 rounded-lg p-3 border border-purple-200">
                            <div className="space-y-2 text-xs">
                              <div className="flex justify-between">
                                ...
                                <span className="font-bold text-purple-900">{formatCurrency(request.original_total)}</span>
                              </div>
                              <div className="flex justify-between">
                                <span className="text-purple-700 font-medium">New Total:</span>
                                <span className="font-bold text-purple-900">{formatCurrency(request.new_total)}</span>
                              </div>
                              ...
                              <div className="flex justify-between">
                                ...
                                <span className={`font-bold ${getPercentageColor(request.cost_increase_percentage)}`}>
                                  +{request.cost_increase_percentage.toFixed(1)}%
                                </span>
                              </div>
                              {request.requires_client_approval && (
                    ...
                    {filteredRequests.map((request, index) => (
                      ...
                      >
                        ...
                        <div className="px-4 pb-3 space-y-1.5 text-xs">
                          <div className="flex justify-between">
                            <span className="text-gray-500">Additional Cost:</span>
                            <span className="font-bold text-blue-600">{formatCurrency(request.additional_cost)}</span>
                          </div>
                          <div className="flex justify-between">
                            <span className="text-gray-500">Cost Increase:</span>
                            <span className="font-semibold text-blue-600">+{request.cost_increase_percentage.toFixed(1)}%</span>
                          </div>
                        </div>
